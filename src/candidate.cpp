...
/*
//TODO: probably remove too short candidates again, as it is very likely, that the reverse can cover more.
void CandidateSet::computeCandidates() {
...
//#pragma parallel for
    for(auto & col : freespaces){
        for(auto& fs : col){
            fs.identifyImportantUpDownYs();
            //for(int x = fs.xSize()-1;x>=0;x--)
            //    for(int y = fs.ySize()-1;y>=0;y--)
            //        fs.identifyImportantUpDownYs(x,y);
        }
    }
...
#pragma omp parallel for default(none) shared(std::cout, totalcount)
    for(int baseCurveIdx = 0; baseCurveIdx < freespaces.size();baseCurveIdx++){
#else
    int loopID = 0;
    for(int baseCurveIdx = 0; baseCurveIdx < freespaces.size();baseCurveIdx++){
                        ...
                        if(!ff1.getCell(edgeIdxT1, edgeIdxB)->importantDownYs.empty()){
                            ...
                            for (std::pair<CellPoint, FreeSpacePoint> paramPointPair: startCellInFF1->importantDownYs) {
                                ...
                                while (startOnT2 < ff2.xSize()) {
                                    ...
                                    //controversal
                                    ParamPoint max = {ff2.ySize()-1,1.0};
                                    if(limit != max && localExtremes.back().y == limit)
                                    {
                                        continue;
                                    }
                        ...
                        if(!ff1.getCell(edgeIdxT1, edgeIdxB)->importantUpYs.empty()){
                            ...
                            for (std::pair<CellPoint, FreeSpacePoint> paramPointPair: startCellInFF1->importantUpYs) {
                                ...
                                while (startOnT2 < ff2.xSize()) {
                                    ...
                                    //controversal
                                    ParamPoint min = {0,0.0};
                                    if(limit != min && localExtremes.back().y == limit)
                                    {
                                        continue;
                                    }
...
#pragma omp critical
        std::cout << baseCurveIdx << ": Finished handling candidates on curve " << baseCurveIdx << ". Time difference = " << std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count() << "[µs]" << std::endl;
        ...
        for(Candidate& candidate : basecurveCandidates){
            ...
            this->operator[](baseCurveIdx).push_back(candidate);
        }
#pragma omp critical
        totalcount += this->operator[](baseCurveIdx).size();
        end = std::chrono::steady_clock::now();
#pragma omp critical
        std::cout << baseCurveIdx << ": Finished computing covering of "<< this->operator[](baseCurveIdx).size() <<" candidates. Time difference = " << std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count() << "[µs]" << std::endl;

    }
    updateConcurrentWeights();
...
void CandidateSet::showCandidates() {
    for (int baseCurveId=0;baseCurveId < freespaces.size();baseCurveId++){
        ...
        for(Candidate & candidate : this->operator[](baseCurveId)){
...
void CandidateSet::showCovering(std::vector<std::pair<int, int>> indices) {
    ...
    int currentId = indices.front().first;
    for(auto index:indices){
        if(currentId != index.first){
            //show
            for(int i = 0; i<freespaces[currentId].size();i++){//auto fs : freespaces[currentId]){
                FreeSpaceVisualizer fsv(freespaces[currentId][i]);
                fsv.showCandidates(candidates,i);
            }
            candidates.clear();
            currentId = index.first;
        }
        candidates.push_back(getCandidate(index));
    }
...
bool mpz_comp4(Candidate& c, mpz_t rand){
    return mpz_cmp(rand,c.cW)>0;//c.concurrentWeight < rand;
}

std::pair<int, int> CandidateSet::draw(mpz_t rand) {
    assert(mpz_cmp(tW,rand)>0);
    auto it = std::lower_bound(begin(),end(),rand,mpz_comp3);
    assert(it < end());
    int idx = it - begin();
    auto it2 = std::lower_bound(this->operator[](idx).begin(), this->operator[](idx).end(),rand,mpz_comp4);
    return {idx,it2-this->operator[](idx).begin()};
}

void CandidateSet::updateConcurrentWeightsForRedraw() {
        //totalWeight = 0;
        mpz_set_ui(tW,0);
        for(auto & col : *this){
            for(auto & candidate : col){
...
void CandidateSet::freshenWeights() {
    for(auto & col : *this){
        for(auto & candidate : col){
            mpz_set(candidate.w,candidate.wback);
        }
    }
}

std::pair<int, int> CandidateSet::drawWithoutRedraw(__mpz_struct *rand) {
    assert(mpz_cmp(tW,rand)>0);
    auto it = std::lower_bound(begin(),end(),rand,mpz_comp3);
    assert(it < end());
    int idx = it - begin();
