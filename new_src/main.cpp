...
bool smallDistanceAt(distance_t t, const Point& ap, distance_t ra, const Point& cp, const Point& dp, distance_t rc, distance_t rd){
    double val = ap.dist_sqr(cp * (1. - t) + dp * t);
    return val <= (ra + (rc*(1.-t))+(rd*t))*(ra + (rc*(1.-t))+(rd*t));
}

Interval intersectionInterval( const Point& ap, distance_t ra, const Point& cp, const Point& dp, distance_t rc, distance_t rd, Interval* outer){
    ...
    const distance_t a = cd.length_sqr() - ((rd-rc)*(rd-rc));
    const distance_t b = cs.dot(cd) - ((rc+ra)*(rd-rc));
    const distance_t c = cs.length_sqr() - ((rc+ra)*(rc+ra));

    distance_t mid = - b / a;
    distance_t discriminant = (mid*mid) - (c / a);

    const bool smallDistAtZero = smallDistanceAt(0.,ap,ra,cp,dp,rc,rd);
    const bool smallDistAtOne = smallDistanceAt(1.,ap,ra,cp,dp,rc,rd);
    bool smallDistAtMid = smallDistanceAt(mid,ap,ra,cp,dp,rc,rd);
    ...
    if (smallDistAtZero) {
        ...
        if (innershift >= outershift && smallDistanceAt(innershift,ap,ra,cp,dp,rc,rd) && !smallDistanceAt(outershift,ap,ra,cp,dp,rc,rd)) {
            begin = innershift;
            ...
            while (right - left > save_eps) {
                ...
                if (smallDistanceAt(m,ap,ra,cp,dp,rc,rd)) { right = m; }
                else { left = m; }
            }
            begin = right;
    ...
    if (smallDistAtOne) {
        ...
        if (innershift <= outershift && smallDistanceAt(innershift,ap,ra,cp,dp,rc,rd) && !smallDistanceAt(outershift,ap,ra,cp,dp,rc,rd)) {
            ...
            while (right - left > save_eps) {
                distance_t m = 0.5 * (left + right);
                if (smallDistanceAt(m,ap,ra,cp,dp,rc,rd)) { left = m; }
                else { right = m; }
            }
            end = left;
    ...
    assert(smallDistanceAt(begin,ap,ra,cp,dp,rc,rd));
    assert(smallDistanceAt(end,ap,ra,cp,dp,rc,rd));
    assert(0. <= begin && begin <= end && end <= 1.);

    assert(outer == nullptr || outer->begin < 0. || !smallDistanceAt(outer->begin,ap,ra,cp,dp,rc,rd));
    assert(outer == nullptr || outer->end > 1. || !smallDistanceAt(outer->end,ap,ra,cp,dp,rc,rd));
...
bool smallDistanceAt(distance_t t,const Point& a,const Point& b, distance_t ra, distance_t rb,const Point& c,const Point& d, distance_t rc, distance_t rd){
    Point p = c*(1.-t) + d*t;
    ...
    return (abab+((rb-ra)*(ra-rb)))*((abab*papa) - (2*abpa*abpa) + (abpa*abpa))<=((abab*(rp+ra)) + ((rb-ra)*abpa))*((abab*(rp+ra)) + ((rb-ra)*abpa));
}

Interval intersectionInterval(const Point& a, const Point& b, distance_t ra, distance_t rb, const Point& c, const Point& d, distance_t rc, distance_t rd, Interval* outer){
    ...
    if(ra < rb){
        return intersectionInterval(b,a,rb,ra,c,d,rc,rd,outer);
    }
    ...
    if(ab.length_sqr() < (ra-rb)*(ra-rb)){
        return intersectionInterval(a,ra,c,d,rc,rd,outer);
    }
...
Interval trueIntersection(const Point& a,const Point& b, distance_t ra, distance_t rb,const Point& c,const Point& d, distance_t rc, distance_t rd, Interval* outer){
    Interval outerCylinderInterval;
    Interval cylinderInterval = intersectionInterval(a,b,ra,rb,c,d,rc,rd,&outerCylinderInterval);
    Interval outerStartInterval;
    Interval startInterval = intersectionInterval(a,ra,c,d,rc,rd,&outerStartInterval);
    Interval outerEndInterval;
    Interval endInterval = intersectionInterval(b,rb,c,d,rc,rd,&outerEndInterval);

    Interval result;
    //verify start
    Point pCylinder = c + (d-c)*cylinderInterval.begin;
    distance_t pR = rc + (rd-rc)*cylinderInterval.begin;
    bool test = smallDistanceAt(cylinderInterval.begin,c,d,rc,rd,a,b,ra,rb);
    if(intersectionInterval(pCylinder,pR,a,b,ra,rb,nullptr).is_empty()) {
    ...
    if(intersectionInterval(qCylinder,qR,a,b,ra,rb,nullptr).is_empty()) {
...
void intersectionprimitivetest(){
    ...
    for(int i=0;i<100;i++) {
        ...
        Interval intersection = trueIntersection(a, b, ra, rb, c, d, rc, rd, nullptr);
    ...
    */
        ...
        double t1 = /*mid - sqrt(discriminant);*/intersection.begin;
        double t2 = /*mid + sqrt(discriminant);*/intersection.end;
