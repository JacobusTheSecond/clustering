...
%     end
    ...
    if ~isempty(mot.rotationQuat)
        if size(rootRot,2)>1
            res.deltaRootPos = [ [0;0;0] C_quatrot(mot.rootTranslation(:,2:end)-mot.rootTranslation(:,1:end-1),quatinv(rootRot(:,2:end)))];
            res.deltaRootOri = [ [1;0;0;0] quatmult(rootRot(:,2:end),quatinv(rootRot(:,1:end-1)))];
