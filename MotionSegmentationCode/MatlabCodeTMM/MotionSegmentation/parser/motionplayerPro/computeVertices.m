...
rnode = find(strncmp('root',skel.jointNames,4));

localSystems         = cell(skel.njoints,1);
localSystems{rnode(1)}  = mot.rotationQuat{rnode(1)};
jointTrajectories    = cell(skel.njoints,1);

jointTrajectories{rnode(1)} = mot.rootTranslation;
...
for i=1:size(skel.paths,1)
    for j=2:numel(skel.paths{i})
        ...
        if isempty(mot.rotationQuat{joint})
            ...
            localSystems{joint,1} = C_quatmult(real(localSystems{pred}),real(mot.rotationQuat{joint}));
        end
