...
for type=1:2
    for trial_idx=1:numel(trial_range)
        ...
        if type == 1
            ...
            options.use_fast_dag = false;
            options.use_create_dag_per_range = false;
            [mot, mots, submots, comps, cuts, subcuts, subcuts_main, subcuts_mirror, timings] = tw_segmentation(skel, mot, options);
            second_timings_results{trial_idx} = timings;
            frame_counts(trial_idx) = mot.nframes;
            second_names{trial_idx} = strcat('Old_', name);
        end 
    end
end
...
figure(1);
timings_table = tw_timingsComparePlot(first_timings_results, second_timings_results, frame_counts, first_names, second_names, 'Average', 'Old_Average', 'Compare DAG Creation');