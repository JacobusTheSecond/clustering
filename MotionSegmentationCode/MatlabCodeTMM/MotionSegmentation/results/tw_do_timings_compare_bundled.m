...
for type=1:2
    for trial_idx=1:numel(trial_range)
        ...
        if type == 1
            ...
            frame_counts(trial_idx) = mot.nframes;
            second_names{trial_idx} = strcat({'BF '}, name);
        end 
    end
end
...
figure(1);
timings_table = tw_timingsComparePlotBundled(first_timings_results, second_timings_results, frame_counts, first_names, second_names, 'Average', 'BF Average', 'Normal Version vs. Bundled Features (BF) Version');