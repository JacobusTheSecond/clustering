...
offset_a_radius = 12;
step_a_radius = 0.5;
radius_a_range = ref_a_radius:step_a_radius:(ref_a_radius + offset_a_radius);

deviation_results = zeros(numel(trial_range), numel(radius_a_range));
ratio_results = zeros(numel(trial_range), numel(radius_a_range));
activity_count_results = zeros(numel(trial_range), numel(radius_a_range));
...
for radius_idx=1:numel(radius_a_range)
    ...
    for trial_idx=1:numel(trial_range)
        ...
        activity_count_results(trial_idx, radius_idx) = activity_count / 25;
    end
end

deviation_eval = sum(deviation_results, 1) / numel(trial_range);
ratio_eval = sum(ratio_results, 1) / numel(trial_range);
activity_count_eval = sum(activity_count_results, 1) / numel(trial_range);

%% visualization

figure(1);
plot(radius_a_range, ratio_eval, 'r', radius_a_range, deviation_eval, 'b', radius_a_range, activity_count_eval, 'g', 'LineWidth', 2);
xlabel('Radius');
legend('Segmentation Ratio', 'Relative Deviation', 'Relative Activity Count');
ax = gca;
