function [] = tw_mergePlot(frame_count, radius, nnidx, nndists, nnidx_mirror, nndists_mirror, nnidx_merge, nndists_merge)
    ...
    for i=1:frame_count
        idx = ~isnan(nnidx_merge(:, i));
        immat_merge(i, nnidx_merge(idx, i)) = nndists_merge(idx, i);
    end
    ...
    gray_map = vertcat(gray(idx_range), nan_color);
    mirror_map = vertcat(autumn(idx_range), nan_color);
